...
class Advert extends Component {
  ...
  render() {
    ...
    if (favorites.list.indexOf(advert._id) !== -1)
      inFavorites = true
          ...
          <div className={s.container}>
              ...
              <div className={s.actionBtns}>
                <TransitionGroup component={null}>
                  <CSSTransition key={inFavorites}
                    ...
                    timeout={300}>
                    <span className={s.actionBtns__btn + (inFavorites ? ' ' + s.isActive : '')} onClick={this.handleClick}>
                      <i className={i.icon}>
            ...
            <div className={c.infoWrapper}>
              <div className={c.infoTags}>
                ...
                {
                  advert.whom
                    ? Object.keys(advert.whom).map((val, i) => (<div className={c.tag + " " + c['tag_' + val]} key={i}>{ formatVal(val, 'whom') }</div>))
                    : null
                }
              </div>
              <div className={c.infoTags}>
                ...
                {
                  advert.tags
                    ? Object.keys(advert.tags).map((val, i) => (<div className={c.tag + " " + c['tag_' + val]} key={i}>{ formatVal(val, 'tags') }</div>))
                    : null
                }
              </div>
              <div className={c.infoTags}>
            ...
            <div className={s.userInfo}>
              ...
              <div className={s.userInfo__info}>
                ...
                <span className={s.userInfo__date + (!advert.createdAt ? ' ' + s.userInfo__date_dummy + ' ' + s.dummy : '')}>{ advert.createdAt ? formatDate(new Date(advert.createdAt)) : ''}</span>
              </div>
            </div>
          </div>
      </CSSTransition>
